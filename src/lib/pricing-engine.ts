...
export function calculateFinalRetailPrice(rawCostUsd: number, exchangeRate: number, currency: string = 'USD', userDiscount: number = 0): number {
  const margin = calculateTieredMargin(rawCostUsd);
  let retailUsd = rawCostUsd + (rawCostUsd * margin);
