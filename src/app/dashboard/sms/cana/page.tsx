...
export default function CANAPurchasePage() {
  const region = "na"; // Hardcoded to North America
  ...
  const fetchLivePrice = async (serviceName: string, cur: string) => {
    ...
    try {
      const res = await fetch('/api/pricing/live', {
        ...
        body: JSON.stringify({ country: "canada", serviceName, currency: cur })
      });
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('realtime-rentals-cana')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'rentals', filter: `region=eq.canada` }, (payload) => {
