...
export default function USPurchasePage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('realtime-rentals-usa')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'rentals', filter: `region=eq.usa` }, (payload) => {
  ...
  const fetchLivePrice = async (serviceName: string, selectedCurrency: string) => {
    ...
    try {
      const res = await fetch('/api/pricing/live', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ country: region, serviceName, currency: selectedCurrency })
      });
      const data = await res.json();
      if (res.ok && data.available && data.cost !== null) {
                                ...
                                {SERVICES.filter((s: any) => s.name.toLowerCase().includes(serviceSearchQuery.toLowerCase()))
                                  ...
                                  .map((service: any) => (
                                    <button
                                      ...
                                      className={`w-full text-left flex justify-between px-4 py-3 rounded-lg text-sm transition-colors ${selectedServiceName === service.name ? "bg-brand-blue/10 dark:bg-brand-blue/20 text-brand-blue font-bold" : "text-slate-600 dark:text-white/70 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white"}`}
                                    >
