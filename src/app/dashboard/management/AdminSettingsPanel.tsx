...
export default function AdminSettingsPanel({ initialMargin }: { initialMargin: number }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/settings', {
        ...
        body: JSON.stringify({ profit_margin: databaseMargin })
      });
      ...
      setMessage({ text: `Profit margin successfully updated to ${parsedMargin}%.`, type: "success" });
    } catch (err: unknown) {
  ...
  return (
    <div className="bg-white dark:bg-[#111111] border border-black/5 dark:border-white/5 rounded-[24px] p-6 md:p-8 shadow-sm dark:shadow-none">
      <div className="flex items-center gap-3 mb-8 pb-6 border-b border-black/5 dark:border-white/5">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">Global Pricing Engine</h2>
          <p className="text-slate-500 dark:text-white/40 text-sm">Adjust markup margins across all API providers.</p>
        </div>
      </div>
      ...
      <div className="max-w-md">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <button 
            onClick={handleSave}
            disabled={isSaving || marginInput === (initialMargin * 100).toString()}
            className="bg-brand-blue text-white px-8 py-3.5 rounded-xl font-bold hover:bg-brand-blue/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2 min-w-[140px] justify-center shadow-md dark:shadow-none"
          >
            {isSaving ? <Spinner className="animate-spin w-5 h-5" /> : "Deploy"}
          </button>
        </div>
